...
int main(int argc, char *argv[])
{
    ...
    if(argc == 1) {
        ...
        std::cout << "Options:" << std::endl
                  ...
                  << "\t-b - be more verbose (basically print all measurements)" << std::endl
    ...
    std::shared_ptr<SRPV::VerifInterface> recognizer = SRPV::VerifInterface::getImplementation();
    std::cout << "  Initializing: ";
    elapsedtimer.start();
    SRPV::ReturnStatus status = recognizer->initialize(apiresourcespath.toStdString());
    ...
    for(int i = 0; i < subdirs.size(); ++i) {
        ...
        if(static_cast<uint>(_files.size()) >= minfilespp) {
            ...
            for(size_t j = 0; j < etpp; ++j) {
                ...
                soundrec = readSoundRecord(_subdir.absoluteFilePath(_files.at(j)),verbose);
                elapsedtimer.start();
    ...
    for(int i = 0; i < distractorfiles.size(); ++i) {
        ...
        std::vector<uint8_t> _templ;
        soundrec = readSoundRecord(indir.absoluteFilePath(distractorfiles.at(i)),verbose);
        elapsedtimer.start();
        status = recognizer->createTemplate(soundrec,SRPV::TemplateRole::Verification_11,_templ);
        vtgentime += elapsedtimer.nsecsElapsed();
        vtemplates[vtpos++] = BiometricTemplate(label,SRPV::TemplateRole::Verification_11,std::move(_templ));
